	...
	<div class="table-responsive mt-2">
		<table aria-label="Data Pasien" id="myTable" class="table table-striped jambo_table bulk_action">
			...
			<tbody>
				...
				$sql = mysqli_query($koneksi, "SELECT * FROM pasien WHERE status = '1' ORDER BY no_rm ASC ") or die(mysqli_error($koneksi));
					...
					while ($data = mysqli_fetch_assoc($sql)) {
						...
						<tr>
							...
							<td>
								...
								<a href="dashboard.php?page=delete_pasien&no_rm=' . $data['no_rm'] . '" class="btn btn-danger btn-sm" onclick="return confirm(\'jika ada data yang berkaitan pada transaksi peminjaman akan dihapus juga , Yakin ingin menghapus data ini?\')">Delete</a>
							</td>
						</tr>
						';
						// $no++;

...
<center>
	<font size="6">Tambah Data Peminjam</font>
</center>
<hr>
<?php

		$no_pj = mysqli_query($koneksi, "SELECT * FROM peminjam") or die(mysqli_error($koneksi));
		if (mysqli_num_rows($no_pj) == 0) {
			$no_pj = 'PJ001';
		} else {
			
			$last_pj = false;
			foreach ($no_pj as $key => $value) {
				$last_pj = $value['kd_peminjam'];
			}
			(int)$last_pj = substr($last_pj, 2);
			$last_pj++;
			$num_padded = sprintf("%03d", $last_pj);
			(string)$no_pj = "PJ" . $num_padded;
		}
...
<form action="dashboard.php?page=tambah_peminjam" method="post">
	...
	<div class="item form-group">
		...
		<div class="col-md-6 col-sm-6">
			<input type="number" name="no_telp" min="0" maxlength="15"
				onkeypress="return (event.charCode == 8 || event.charCode == 0) ? null : event.charCode >= 48 && event.charCode <= 57"
				oninput="maxLengthCheck(this)" class="form-control" required>
		</div>
		<script>
